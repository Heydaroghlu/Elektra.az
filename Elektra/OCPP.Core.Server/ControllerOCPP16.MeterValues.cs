    ...
    public partial class ControllerOCPP16
    {
        public string HandleMeterValues(OCPPMessage msgIn, OCPPMessage msgOut)
        {
            ...
            try
            {
                ...
                Logger.LogTrace("isameter"+msgIn.JsonPayload);
                Logger.LogTrace("isa Connector" + meterValueRequest.ConnectorId);

                Logger.LogTrace("MeterValues => Message deserialized");

               if(meterValueRequest.ConnectorId!=null &&meterValueRequest.ConnectorId>0)
                {
            ...
            catch (Exception exp)
            {
                ...
                Logger.LogError(exp, "MeterValues isa => Exception: {0}", exp.Message);
