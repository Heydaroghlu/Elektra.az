...
namespace OCPP.Core.Server
{
    public partial class OCPPMiddleware
    {
        ...
        public async Task Invoke(HttpContext context, OCPPCoreContext dbContext)
        {
            ...
            {
                // Sadece POST ve PUT isteklerinde gövdeyi oku
                    ...
                    else if (cmd == "RemoteStartTransaction")
                    {
                        ...
                        if (!string.IsNullOrEmpty(urlChargePointId))
                        {
                            try
                            {
                                ...
                                if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                                {
                                    ChargeStartDTO chargeStart = new ChargeStartDTO();
                                    if (body!=null)
                                    {
                                        chargeStart = JsonConvert.DeserializeObject<ChargeStartDTO>(body);
                                    }
                                    // Send message to chargepoint
                        ...
                        if (!string.IsNullOrEmpty(urlChargePointId))
                        {
                            try
                            {
                                ...
                                if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                                {
                                    ...
                                    ChargeStopDTO chargeStop = new ChargeStopDTO();
                                    if (body != null)
                                    {
                                        chargeStop = JsonConvert.DeserializeObject<ChargeStopDTO>(body);
                                    }
                                    // Send message to chargepoint
                                    if (status.Protocol == Protocol_OCPP20)
                                    {
